 ...
 */
int history_lengths[NHIST+1]={0,8,12,18,27,40,60,90,135,203,305,459,690};
 ...
 */
int history_lengths[NHIST+1]={0,6,10,18,25,35,55,69,105,155,230,354,479,642,1012,1347};
...
{  
    ...
    int entry_width = (predictor_counter_width + useful_counter_width + entry_tag_width);
    return entry_width;
}

static inline int num_entries(int bank_number)
{
  return  (1 << (logg[bank_number]));
}

/**
 * Each table's entry size and number of entries is hand tuned.
 */
static inline int tage_table_size(int table_number)
{
  return num_entries(table_number) * entry_width(table_number);
}
...
public:
  ...
  void reinit () {
    ...
    for (int i = 1; i <= NHIST; i++) {
	ch_i[i].init (history_lengths[i], (logg[i]), i - 1);
