...
class PREDICTOR {
 ...
 public:
  ...
  void initSC();
  void HistoryUpdate (uint32_t PC, uint8_t brtype, bool taken, uint32_t target, int &Y,
  ...
  void UpdateChoseSC(UINT32 PC, bool taken, bool PRED);
  void UpdateSC(UINT32 PC, bool taken, bool PRED);
  bool ChoseSCpredict(UINT32 PC, bool Tpred);
  bool SCpredict(UINT32 PC, bool Tpred);
  ...
  int Gpredict (UINT32 PC, long long BHIST, int *length, int8_t ** tab, int NBR);
  void Gupdate (UINT32 PC, bool taken, long long BHIST, int *length, int8_t ** tab, int NBR,int WIDTH);
